# From One Lens to Another
By Ali Manan (sidipasteles)

Welcome to the official repository for **From One Lens to Another**. This repository houses both my personal memoir draft and standalone software projects. 

---

## 📖 The Memoir
This project is a work in progress. I update this file regularly as new chapters are completed.

👉 **[Read the Autobiography Draft (PDF)](./autobiography.pdf)**

### ⚖️ Book Copyright Notice
**Copyright © 2026 Ali Manan. All Rights Reserved.**  
The text of this autobiography is the sole intellectual property of the author. No part of this manuscript may be copied, reproduced, redistributed, or modified in any form without explicit written permission. 

### ⚖️ DISCLAIMER
**Please read this section before proceeding with the book.**  
This eBook is an artistic display of the author’s personal recollection, within various settings, which reflects the author’s own memories, interpretations, 
and emotions.  

While the author has written to the best of his recollection, the author recognizes that memory and perspectives are subjective, and others may recall these events differently. The views expressed here are either his own or creatively embedded, and are not intended as objective fact, nor as an attempt to defame or harm any individual or institution.


---

## 💻 Accompanying Code Projects
The source code and developer tools associated with this project are organized neatly inside the projects directory.

👉 **[Browse the Code Projects](./projects/)**

### 📄 Code License
All source code files located inside the `/projects/` folder are open-source and licensed under the **GNU General Public License v3.0 (GPL-3.0)**. You are free to use and modify the code, provided you credit the author and release any derivative projects under the same GPL-3.0 open-source license. 
*(Note: This license applies exclusively to the code files and does not extend to the autobiography text.)*
